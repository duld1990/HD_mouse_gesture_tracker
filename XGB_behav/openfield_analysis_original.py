...
def classify_behaviors_with_xgboost(df_valid, xgb_model, scaler, label_encoder, feature_names, sampling_rate):
    ...
    Args:
        df_valid: DataFrame with valid tracking data
        xgb_model: Trained XGBoost model
        scaler: Fitted StandardScaler
        label_encoder: Fitted LabelEncoder
        feature_names: List of feature names used in training
        sampling_rate: Video sampling rate (fps)
    
    Returns:
        DataFrame with XGBoost predictions and confidence scores
    """
    ...
    try:
        ...
        # Handle missing values in features (interpolate small gaps)
...
def classify_behaviors_original(df_valid, valid_mask, sampling_rate):
    """
    Classify mouse behaviors using original logic (head angle, speed, distance).
    
    Args:
        df_valid: DataFrame with valid tracking data
        valid_mask: Boolean mask indicating valid rows
        sampling_rate: Video sampling rate (fps)
    
    Returns:
        DataFrame with original logic predictions and confidence scores
    """
    ...
    for i in range(total_frames):
        if valid_mask[i]:
            ...
            if np.isnan(axis_x) or np.isnan(axis_y) or np.isnan(head_vec_x) or np.isnan(head_vec_y):
                continue
            ...
            speed[i] = np.sqrt(body_dx[i]**2 + body_dy[i]**2) * sampling_rate
            distance_to_forelimb[i] = np.sqrt((body_x[i] - forelimb_x[i])**2 + (body_y[i] - forelimb_y[i])**2)
    
    # Apply original logic thresholds
    ...
    freezing_mask = speed <= walk_speed_threshold
    
    # Combine original logic predictions
    original_predictions[head_turn_mask] = 'head_turn'
    original_predictions[walking_mask] = 'walk'
    original_predictions[freezing_mask] = 'freeze'
    
    # Calculate confidence scores for original logic
    # For head turn, confidence is based on angle
    head_turn_confidence = np.full(total_frames, 0.0)
    head_turn_confidence[head_turn_mask] = 1.0 # 100% confidence for head turns
    
    # For walking/freezing, confidence is based on speed
    walk_freeze_confidence = np.full(total_frames, 0.0)
    walk_freeze_confidence[walking_mask] = 1.0 # 100% confidence for walking
    walk_freeze_confidence[freezing_mask] = 1.0 # 100% confidence for freezing
    
    # For stand behavior, confidence is based on distance to forelimb
    stand_confidence = np.full(total_frames, 0.0)
    stand_rearing_mask = distance_to_forelimb < rearing_dist_thresh
    stand_all_close_mask = (distance_to_forelimb < stand_dist_thresh) & (distance_to_forelimb < rearing_dist_thresh) # Combined mask
    
    stand_confidence[stand_rearing_mask] = 1.0 # 100% confidence for rearing
    stand_confidence[stand_all_close_mask] = 1.0 # 100% confidence for all-close
    
    # Combine all original logic predictions and confidence scores
    original_predictions_combined = np.full(total_frames, 'other')
    original_confidence_combined = np.full(total_frames, 0.0)
    
    # Prioritize XGBoost predictions if available
    if xgb_model is not None and scaler is not None and label_encoder is not None:
        xgb_results = classify_behaviors_with_xgboost(df_valid, xgb_model, scaler, label_encoder, feature_names, sampling_rate)
        if xgb_results is not None:
            original_predictions_combined[xgb_results['frame_index'].values] = xgb_results['xgb_prediction'].values
            original_confidence_combined[xgb_results['frame_index'].values] = xgb_results['xgb_confidence'].values
    
    # Fallback to original logic if XGBoost is not available or failed
    original_predictions_combined[head_turn_mask] = 'head_turn'
    original_predictions_combined[walking_mask] = 'walk'
    original_predictions_combined[freezing_mask] = 'freeze'
    original_predictions_combined[stand_rearing_mask] = 'stand'
    original_predictions_combined[stand_all_close_mask] = 'stand'
    
    # Combine original logic confidence scores
    original_confidence_combined[head_turn_mask] = head_turn_confidence
    original_confidence_combined[walking_mask] = walk_freeze_confidence
    original_confidence_combined[freezing_mask] = walk_freeze_confidence
    original_confidence_combined[stand_rearing_mask] = stand_confidence
    original_confidence_combined[stand_all_close_mask] = stand_confidence
    
    # Create results DataFrame for original logic
    original_results_df = pd.DataFrame({
        'frame_index': df_valid.index,
        'original_prediction': original_predictions_combined,
